...
progressBar.on('SIGINT', () => {
  console.log('\ncli-progress caught SIGINT');
});
...
export async function runBarrelBreaker(
  ...
  // Dynamically set progress bar width based on terminal width.
  const terminalWidth = process.stdout.columns || 80;
  const barWidth = Math.max(terminalWidth - 20, 10);
  ...
  for (let i = 0; i < totalFiles; i++) {
    ...
    logger.debug(chalk.bold.green(`\n=== Processing File: ${path.relative(cwd, sourceFile.getFilePath())} ===`));
    const fileUpdates = rewriteImportsInFile(sourceFile, {
      project,
      aliasPrefixes,
      paths: pathsConfig,
      isDryRun,
    ...
    logger.debug(indent(`Finished processing file. Updates made: ${fileUpdates}`, 2));
    // Update progress bar.
  ...
  if (summary) {
    ...
    logger.info(indent(`Total import declarations updated: ${totalUpdates}`, 2));
