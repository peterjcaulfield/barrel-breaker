...
const getLogger = (verbose = false, colorsEnabled = false) => {
  if (!loggerInstance) {
    loggerInstance = pino(
      {
  ...
  if (rewritten.named.length > 0) {
    const namedPart = rewritten.named
      .map(([name, alias, isTypeOnly]) => {
        const base = name === alias ? name : `${name} as ${alias}`;
        return isTypeOnly ? `type ${base}` : base;
      })
      .join(", ");
...
export async function runBarrelBreaker(
  ...
  const logger = getLogger(verbose, colorsEnabled);
  ...
  if (fs.existsSync(tsconfigPath)) {
    ...
    logger.warn(`Warning: tsconfig.json not found at ${tsconfigPath}. Be sure to pass a path to the tsconfig.json via --tsconfig if you are executing in a Typescript project. Proceeding with default configuration.`);
  ...
  if (fs.existsSync(tsconfigPath)) {
    ...
    logger.warn(`Warning: tsconfig.json not found at ${tsconfigPath}. Proceeding with default configuration.`);
    project = new Project();
  }
...
function rewriteImportsInFile(sourceFile, { project, aliasPrefixes, paths, isDryRun, tsconfigDir, verbose, logger }) {
  ...
  sourceFile.getImportDeclarations().forEach((importDecl) => {
    ...
    if (moduleSpecifier.startsWith(".")) {
      isLocal = true;
    } else if (aliasPrefixes.some(prefix => moduleSpecifier.startsWith(prefix))) {
      isLocal = true;
    } else if (baseUrl) {
      // Resolve the candidate path relative to tsconfigDir.
      // (If baseUrl is ".", this is equivalent to the project root.)
      const candidatePath = path.resolve(tsconfigDir, moduleSpecifier);
      // Check if candidatePath (or candidatePath.ts, candidatePath.tsx, or candidatePath/index.ts) exists.
      if (fs.existsSync(candidatePath) ||
        fs.existsSync(candidatePath + ".ts") ||
        fs.existsSync(candidatePath + ".tsx") ||
        fs.existsSync(path.join(candidatePath, "index.ts")) ||
        fs.existsSync(path.join(candidatePath, "index.tsx"))) {
        isLocal = true;
      }
    }
    ...
    if (result.changed) {
      ...
      for (const { specifier, named, defaultImport } of result.rewrittenImports) {
        ...
        named.forEach(([name, alias, isTypeOnly]) => {
          importGroups[specifier].named.set(name, { alias, isTypeOnly });
        });
      }
    }
  });

  // TODO: verify if we can return here and just use a binary condition (either changed or processed barrel imports)
  ...
  return () => {
    ...
    Object.entries(importGroups).forEach(([specifier, { named, default: def }]) => {
      sourceFile.addImportDeclaration({
        ...
        namedImports: Array.from(named.entries()).map(([name, { alias, isTypeOnly }]) =>
          name === alias ? (isTypeOnly ? { isTypeOnly: true, name } : name)
            : (isTypeOnly ? { isTypeOnly: true, name, alias } : { name, alias })
        ),
        defaultImport: def,
      });
    });
...
function processImportDeclaration({
  ...
  importDecl.getNamedImports().forEach((namedImport) => {
    ...
    const isTypeOnly = typeof namedImport.isTypeOnly === "function" ? namedImport.isTypeOnly() : false;
    // if the resolved filepath for the symbol is different to the original import path declaration rewrite the import
    if (resolved !== moduleSpecifier) {
      if (!rewrittenBySpecifier.has(resolved)) {
        rewrittenBySpecifier.set(resolved, { named: [], defaultImport: undefined });
      }

      rewrittenBySpecifier.get(resolved).named.push([originalExportName, usedName, isTypeOnly]);
      bindingsUpdated++;
      // TODO: verify whether this is redundant. E.g we are always modifying if we enter processImportDeclaration
      changed = true;
    } else {
      // resolved matches the original import declaration so add to unchanged import collection
      // this can be the base when we have "impure" barrel files
      keptNamed.push({ name, alias, isTypeOnly });
    }
  });

  if (changed) {
    if (keptNamed.length > 0) {
      // TODO: move to main write batch
      sourceFile.addImportDeclaration({
        kind: StructureKind.ImportDeclaration,
        moduleSpecifier,
 ...
 * @param {SourceFile} sourceFile - The source file being processed.
 * @param {string[]} originalImportTexts - The original import declaration texts.
 * @param {Record<string, {named: Map<string, { alias: string, isTypeOnly: boolean }>, default: string|undefined}>} importGroups - The new grouped imports.
 */
function printImportDiff(sourceFile, originalImportTexts, importGroups) {
  ...
  const updatedImportText = Object.entries(importGroups)
    .map(([specifier, { named, default: def }]) => {
      // Convert the Map of named imports into an array of tuples: [name, alias, isTypeOnly]
      const namedArr = Array.from(named.entries()).map(
        ([name, { alias, isTypeOnly }]) => [name, alias, isTypeOnly]
      );
      return buildImportString({ specifier, named: namedArr, defaultImport: def });
    })
    .join("\n");
